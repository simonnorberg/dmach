...
/**
 *
 * AudioRecordWrapper is a wrapper for {@link android.media.AudioRecord}.  It is an auxiliary class
 * for {@link AudioWrapper}; the purpose of the bizarre queuing mechanism is to work around the
...
public class AudioRecordWrapper {
    ...
    public AudioRecordWrapper(int sampleRate, int inChannels, int bufferSizePerChannel)
        ...
        if (minRecSizeBytes <= 0) {
            throw new IOException("bad AudioRecord parameters; sr: " + sampleRate + ", ch: "
                    + inChannels + ", bufSize: " + bufferSizePerChannel);
        }
        while (recSizeBytes < minRecSizeBytes) {
            recSizeBytes += bufSizeBytes;
        }
        rec = new AudioRecord(MediaRecorder.AudioSource.MIC, sampleRate, channelConfig, ENCODING,
                recSizeBytes);
        if (rec.getState() != AudioRecord.STATE_INITIALIZED) {
            rec.release();
            throw new IOException("unable to initialize AudioRecord instance for sr: " + sampleRate
                    + ", ch: " + inChannels + ", bufSize: " + bufferSizePerChannel);
        }
    }
