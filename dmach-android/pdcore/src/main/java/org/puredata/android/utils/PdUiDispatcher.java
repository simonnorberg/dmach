...
/**
 * Subclass of {@link org.puredata.core.utils.PdDispatcher} for executing callbacks on the main UI
 * thread of an Android app.  It is actually more general than that; instances of this class will
 * execute their callbacks in whichever thread they were created in, but in practice it really only
 * makes sense to create instances of this class in the main UI thread.
 *
...
public class PdUiDispatcher extends PdDispatcher {
    ...
    @Override
    public synchronized void receiveMessage(final String source, final String symbol,
                                            final Object... args) {
        if (Thread.currentThread().equals(target)) {
