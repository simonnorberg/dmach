...
/**
 *
 * AudioWrapper wraps {@link android.media.AudioTrack} and {@link android.media.AudioRecord} objects
 * and manages the main audio rendering thread. It hides the complexity of working with raw PCM
 * audio; client code only needs to implement a JACK-style audio processing callback (jackaudio.org).
 *
...
public abstract class AudioWrapper {
     ...
     * @throws java.io.IOException if the audio parameters are not supported by the device
     */
    public AudioWrapper(int sampleRate, int inChannels, int outChannels, int bufferSizePerChannel)
        ...
        rec = (inChannels == 0)
                ? null
                : new AudioRecordWrapper(sampleRate, inChannels, bufferSizePerChannel);
        ...
        if (minTrackSizeBytes <= 0) {
            throw new IOException("bad AudioTrack parameters; sr: " + sampleRate +", ch: "
                    + outChannels + ", bufSize: " + trackSizeBytes);
        }
        while (trackSizeBytes < minTrackSizeBytes) {
            trackSizeBytes += bufSizeBytes;
        }
        track = new AudioTrack(AudioManager.STREAM_MUSIC, sampleRate, channelConfig, ENCODING,
                trackSizeBytes, AudioTrack.MODE_STREAM);
        if (track.getState() != AudioTrack.STATE_INITIALIZED) {
            track.release();
            throw new IOException("unable to initialize AudioTrack instance for sr: " + sampleRate
                    + ", ch: " + outChannels + ", bufSize: " + trackSizeBytes);
        }
    }

    /**
     * Main audio rendering callback, reads input samples and writes output samples; inspired by the
     * process callback of JACK
     *
     * Channels are striped across buffers, i.e., if there are two output channels, then
     * outBuffer[0] will be the first sample for the left channel, outBuffer[1] will be the first
     * sample for the right channel, outBuffer[2] will be the second sample for the left channel, etc.
     *
    ...
    /**
     * Start the audio rendering thread as well as {@link android.media.AudioTrack} and
     * {@link android.media.AudioRecord} objects
     *
     * @param context
     */
    ...
    /**
     * Stop the audio thread as well as {@link android.media.AudioTrack} and
     * {@link android.media.AudioRecord} objects
     */
    ...
    /**
     * Release resources held by {@link android.media.AudioTrack} and
     * {@link android.media.AudioRecord} objects;
     ...
     * will throw an exception on older versions
     */
    public synchronized int getAudioSessionId() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD) {
            ...
            throw new UnsupportedOperationException("audio sessions not supported in Android "
