...
/**
 *
 * PdAudio manages an instance of {@link org.puredata.android.io.AudioWrapper} that uses Pure Data
...
public class PdAudio {
     ...
     * @param ticksPerBuffer number of Pure Data ticks (i.e., blocks of 64 samples) per buffer;
     *                       choose 1 for minimal latency, or more if performance is a concern; for
     *                       Java audio only (Android 2.2 or earlier), ignored by OpenSL components
     * @param restart flag indicating whether the audio thread should be stopped if it is currently
     *                running
     * @throws java.io.IOException if the audio parameters are not supported by the device
     */
    public synchronized static void initAudio(int sampleRate, int inChannels, int outChannels,
        ...
        if (PdBase.openAudio(inChannels, outChannels, sampleRate, null) != 0) {
            throw new IOException("unable to open Pd audio: " + sampleRate + ", " + inChannels
                    + ", " + outChannels);
        }
        if (!PdBase.implementsAudio()) {
            if (!AudioParameters.checkParameters(sampleRate, inChannels, outChannels) || ticksPerBuffer <= 0) {
                throw new IOException("bad Java audio parameters: " + sampleRate + ", "
                        + inChannels + ", " + outChannels + ", " + ticksPerBuffer);
            }
