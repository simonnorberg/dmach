...
public class NativeLoader {
    ...
    public static class NativeLibraryLoadError extends UnsatisfiedLinkError {
        ...
        public NativeLibraryLoadError(String message, Throwable cause) {
            // The convenience super-constructor, with cause as second argument, is only available
            // from Java 1.7
            super(message);
            initCause(cause);
        }
    ...
    private static void loadLibraryFromJar(String library) {
        ...
        if (in == null) {
            throw new NativeLibraryLoadError("Couldn't find " + library + " for this platform "
                    + osName + "/" + osArch);
        }
        try {
            File fileOut = File.createTempFile(
                    library.replaceFirst("\\.[^.]*$", ""), library.replaceFirst("^.*\\.", "."));
